 ...
 */
export const createConsignee = async (req: Request, res: Response): Promise<void> => {
  try {
    // Validate request body
    const validatedData = createConsigneeSchema.parse(req.body);

    // Check for duplicate email
    const existingConsignee = await ConsigneeModel.findOne({ email: validatedData.email });
 ...
 */
export const editConsignee = async (req: Request, res: Response): Promise<void> => {
  try {
    // Validate request body
    const validatedData = updateConsigneeSchema.parse(req.body);

    // Find and update the consignee
    const updatedConsignee = await ConsigneeModel.findByIdAndUpdate(req.params.consigneeId, validatedData, {
      new: true,
      runValidators: true,
    });
 ...
 */
export const deleteConsignee = async (req: Request, res: Response): Promise<void> => {
  try {
 ...
 */
export const toggleActiveConsignee = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    send(res,200,`Consignee ${consignee.isActive ? "activated" : "deactivated"} successfully`);
  } catch (error) {
